...
class GuildStaff(ExultCog):
    ...
    config = app_commands.Group(name="config", description="Configure the bot to suit your server's needs!")
    modrole = app_commands.Group(name="modrole", description="Configure the server's custom moderator roles!", parent=config)
    moduser = app_commands.Group(name="moduser", description="Configure the server's custom moderator users!", parent=config)
    adminrole = app_commands.Group(name="adminrole", description="Configure the server's custom admin roles!", parent=config)
    adminuser = app_commands.Group(name="adminuser", description="Configure the server's custom admin users!", parent=config)

    @modrole.command(name="add", description="Make an addition to the server's custom moderator roles!")
    @app_commands.describe(role="The role that you want to add as a custom moderator role.")
    ...
    @app_commands.describe(role="The role that you want to remove as a custom moderator role.")
    ...
    @app_commands.describe(user="The user that you want to remove as a custom moderator user.")
    async def moduser_remove_slash(self, itr: discord.Interaction, user: discord.Member):
        await itr.response.defer()
        bot: ExultBot = itr.client
        followup: discord.Webhook = itr.followup
        member_id = user.id
