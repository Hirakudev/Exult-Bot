from discord import Interaction, Member, Object, Forbidden, HTTPException
from discord.app_commands import describe, Group, Range
#Discord Imports
...
class Purge(ExultCog):
    ...
    async def purge_messages(self, itr: Interaction, limit: int, check, *, before=None, after=None):
        if limit > 2000:
    ...
    purge = Group(name="purge", description="Purge messages in the current channel.")

    @purge.command(name="embeds", description=f"Removes all messages containing embeds in the given range.")
    @describe(messages="Number of messages you want to search through.")
    @permissions(manage_messages=True)
    @guild_staff()
    async def purge_embeds_slash(self, itr: Interaction, messages: Range[int, 1, 2000]):
    ...
    @permissions(manage_messages=True)
    @guild_staff()
    async def purge_files_slash(self, itr: Interaction, messages: Range[int, 1, 2000]):
    ...
    @describe(messages="Number of messages you want to delete.")
    @permissions(manage_messages=True)
    @guild_staff()
    async def purge_messages_slash(self, itr: Interaction, messages: Range[int, 1, 2000]):
    ...
    @describe(user="The user that you want to purge messages from.", messages="Number of messages you want to search through.")
    @permissions(manage_messages=True)
    @guild_staff()
    async def purge_user_slash(self, itr: Interaction, user: Member, messages: Range[int, 1, 2000]):
    ...
    @describe(sequence="The sequence of characters that a message should have to delete it", 
              messages="Number of messages you want to search through.")
    @permissions(manage_messages=True)
    @guild_staff()
    async def purge_containing_slash(self, itr: Interaction, sequence: str, messages: Range[int, 1, 2000]):
        if len(sequence) < 3:
    ...
    @describe(messages="Number of messages you want to search through.", prefix="If this prefix is in a message, delete the message.")
    @permissions(manage_messages=True)
    @guild_staff()
    async def purge_bot_slash(self, itr: Interaction, messages: Range[int, 1, 2000], prefix: str=None):
    ...
    @permissions(manage_messages=True)
    @guild_staff()
    async def purge_emoji_slash(self, itr: Interaction, messages: Range[int, 1, 2000]):
    ...
    @permissions(manage_messages=True)
    @guild_staff()
    async def purge_reaction_slash(self, itr: Interaction, messages: Range[int, 1, 2000]):
        if messages > 2000:
