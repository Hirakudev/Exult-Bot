...
class Case(ExultCog):
    ...
    case = Group(name="case", description="Handle Moderation Cases stored for this server!")

    @case.command(name="display", description="Display information on a Moderation Case from this server.")
    @describe(case_id="The ID of the case you want to view.")
    @guild_staff(manage_messages=True)
    async def case_display_slash(self, itr: Interaction, case_id: int):
    ...
    @describe(case_id="The ID of the case you want to delete.")
    @guild_staff(manage_messages=True)
    @moderation(self_action=True)
    async def case_delete_slash(self, itr: Interaction, case_id: int):
        await itr.response.defer()
        bot: ExultBot = itr.client
        followup: Webhook = itr.followup
        guild = itr.guild
    ...
    @describe(case_id="The ID of the case you want to edit.")
    @guild_staff(manage_messages=True)
    @moderation(self_action=True)
    async def case_edit_slash(self, itr: Interaction, case_id: int):
        bot: ExultBot = itr.client
        guild = itr.guild
