...
class AdaptiveTransformerProxy(Generic[T]):
    ...
    async def autocomplete(
        self, itr: KnownInteraction[ExultBot], value: Union[int, float, str]
    ...
    async def transform(self, itr: KnownInteraction[ExultBot], value: str) -> T:
...
class ProxyTransformer(app_commands.Transformer, Generic[T]):
    ...
    @classmethod
    async def transform(cls, itr: KnownInteraction[ExultBot], value: Any):
        return await cls.PROXY.transform(itr, value)

    @classmethod
    async def autocomplete(cls, itr: KnownInteraction[ExultBot], value: Any) -> Any:  # type: ignore
